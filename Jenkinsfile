pipeline {
    ...
    environment { 
        ...
        registry = "bom.ocir.io/yzguo69kabyn/bkhaire/com.ocsc.poc.order"
    ...
    stages {
        stage('Cloning our Git') {
            steps { 
                git 'https://github.com/bhushankhaire/com.ocsc.poc.order.git'
            }
        } 
        stage('Maven Build') {
            steps {
	...
	stage('Deploy to OKE') {
         /* Deploy the image to OKE*/
            ...
            /*sh "'sudo cp /var/lib/jenkins/workspace/deploy.sh /var/lib/jenkins/workspace/jenkins-oci_master'"*/
                 sh 'sh /var/lib/jenkins/workspace/com.ocsc.poc.order/deploy.sh'
            }
         }
    }
}
